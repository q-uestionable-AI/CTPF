...
qai ipi listen --port 9090

# Remote callbacks: keep bind on loopback and use a tunnel
...
When the listener runs with `--tunnel cloudflare` (see [Remote callbacks via Cloudflare Tunnel](/ipi/callbacks#remote-callbacks-via-cloudflare-tunnel)), a few behaviors differ from a bare loopback listener.
...
For testing against cloud-hosted AI targets, keep the listener on loopback and expose it via tunnel:

- **Local testing:** `http://127.0.0.1:8080` when the agent runs on the same machine
- **Cloud targets:** `qai ipi listen --tunnel cloudflare` and use the printed HTTPS URL as `--callback`

```bash
# Start the listener with a Cloudflare Quick Tunnel
qai ipi listen --tunnel cloudflare

# Use the printed tunnel URL when generating payloads
qai ipi generate --callback https://<tunnel-host>
```

...
The listener binds to `127.0.0.1` by default and rejects non-loopback `--host` values. For remote callbacks use `--tunnel cloudflare` instead of binding to all interfaces.
...
The callback URL embedded in payloads must be reachable from the target system. The listener binds loopback-only (`127.0.0.1` / `localhost` / `::1`); non-loopback `--host` values are rejected. Reachability options:

- **Local testing:** `http://127.0.0.1:8080` (or `http://localhost:8080`) when the agent runs on the same machine
- **Cloud / remote targets:** keep the listener on loopback and use `--tunnel cloudflare` — see [Remote callbacks via Cloudflare Tunnel](#remote-callbacks-via-cloudflare-tunnel) above.
